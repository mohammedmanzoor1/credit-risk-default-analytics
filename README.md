...
Loaded the raw loan applicant dataset in Python and explored its structure
Cleaned the data in Python — handled missing values, standardized formats, and resolved data quality issues
Reviewed the dataset structure and standardized column naming conventions
Created derived analytical fields: Credit Score Band, DTI Band, EMI-to-Income Ratio, Tenure Stability
Loaded the cleaned data into SQL Server and ran queries to:
...
Built an interactive Tableau dashboard with segmentation filters
Documented findings and recommendations in a written report
...
How to Run:
Load the raw loan dataset and run the Python data-cleaning script
Import the cleaned dataset into SQL Server
Run the provided SQL scripts to generate default-rate and segmentation tables
Connect Tableau to the SQL Server output and open the dashboard workbook (.twbx)
Refer to the PDF report and Gamma slides for detailed findings and recommendations
